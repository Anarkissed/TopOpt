...
struct VIn  { float3 position [[attribute(0)]]; float3 normal [[attribute(1)]]; float4 tint [[attribute(2)]]; float4 flags [[attribute(3)]]; };
...
fragment float4 viewer_fragment(VOut in [[stage_in]], constant float4& reveal [[buffer(0)]],
                                ...
                                constant CutUniforms& cut [[buffer(2)]]) {
    if (reveal.w > 0.5) {
    ...
    float4 tint = in.tint;
    // ★ §6 — THE CUT, EXACTLY. On the selected region's faces the side is decided
    // HERE, per fragment, from the fragment's own model position: the boundary is
    // then the plane itself, at any tessellation, and a triangle the plane crosses
    // is split by it rather than being forced whole to one side.
...
final class MeshRenderer: NSObject, MTKViewDelegate {
    ...
    private func encode(into rpd: MTLRenderPassDescriptor, aspect: Float, into cmd: MTLCommandBuffer,
        ...
        // ★ §6 — the cut's half-space, tested per fragment (index 2).
        var cu = cutUniforms
        enc.setFragmentBytes(&cu, length: MemoryLayout<CutUniformsSwift>.stride, index: 2)
