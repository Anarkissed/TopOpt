...
final class ForceModelTests: XCTestCase {
    ...
    // MARK: custom ring direction (M7.6-ring, D4 v2)

    func testCustomDirectionOverridesSnapInForceVector() {
        // A load aimed with the ring stores a world-space custom direction that
        // overrides the Gravity/Push/Pull snap when the solver force vector is built.
        let (_, ids) = groups([1])
        var fm = ForceModel()
        fm.makeLoad(ids[0])                                   // default: .gravity → world −Y
        let custom = simd_normalize(SIMD3<Float>(1, 0, 0))
        fm.setCustomDirection(ids[0], custom)
        XCTAssertEqual(fm.customDirection(for: ids[0]), custom)
        // groupNormal is irrelevant once a custom aim is set — the vector follows it.
        let f = try! XCTUnwrap(fm.loadForceVectorNewtons(ids[0], groupNormal: SIMD3<Float>(0, 0, 1)))
        let dir = simd_normalize(f)
        XCTAssertEqual(dir.x, 1, accuracy: 1e-5)
        XCTAssertEqual(dir.y, 0, accuracy: 1e-5)
        XCTAssertEqual(simd_length(f), Float(fm.forceNewtons(kg: ForceModel.defaultWeightKg)), accuracy: 1e-3)
    }

    func testChoosingASnapClearsTheCustomDirection() {
        // Tapping a snap preset overrides the ring aim (D4: direction changes only
        // via constrained affordances, and the last one chosen wins).
        let (_, ids) = groups([1])
        var fm = ForceModel()
        fm.makeLoad(ids[0])
        fm.setCustomDirection(ids[0], SIMD3<Float>(1, 0, 0))
        XCTAssertNotNil(fm.customDirection(for: ids[0]))
        fm.setDirection(ids[0], .push)
        XCTAssertNil(fm.customDirection(for: ids[0]))
        // Now the force follows Push (−normal) again.
        let f = try! XCTUnwrap(fm.loadForceVectorNewtons(ids[0], groupNormal: SIMD3<Float>(0, 1, 0)))
        XCTAssertEqual(simd_normalize(f).y, -1, accuracy: 1e-5)
    }

    func testCustomDirectionIgnoredForNonLoadAndForBadVector() {
        let (_, ids) = groups([1, 2])
        var fm = ForceModel()
        fm.makeAnchor(ids[0])
        fm.setCustomDirection(ids[0], SIMD3<Float>(1, 0, 0))  // anchors take no aim
        XCTAssertNil(fm.customDirection(for: ids[0]))
        fm.makeLoad(ids[1])
        fm.setCustomDirection(ids[1], SIMD3<Float>(0, 0, 0))  // degenerate → ignored
        XCTAssertNil(fm.customDirection(for: ids[1]))
    }

    func testSyncPrunesCustomDirectionOfRemovedGroups() {
        var m = SelectionModel()
        m.addGroup(); m.pickFaces([1])
        let id = m.groups[0].id
        var fm = ForceModel()
        fm.makeLoad(id)
        fm.setCustomDirection(id, SIMD3<Float>(1, 0, 0))
        m.remove(id)
        fm.sync(groups: m.groups)
        XCTAssertNil(fm.customDirection(for: id))
    }

    func testPanelKindLabelShowsCustomAim() {
        let (_, ids) = groups([1])
        var fm = ForceModel()
        fm.unit = .kg
        fm.makeLoad(ids[0]); fm.setWeight(ids[0], kg: 2.5)
        fm.setCustomDirection(ids[0], SIMD3<Float>(1, 0, 0))
        XCTAssertEqual(fm.panelKindLabel(for: ids[0]), "2.5 kg · custom")
    }
}
