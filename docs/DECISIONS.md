...
- 2026-07-10: Re handoff 030 (Blocked): demo fixture erratum confirmed —
  reviewer's original expected_values geometry was wrong (placement origins
  misread as vertices); the committed l-bracket.step was correct all along.
  Regenerated from topological vertices, matching run 030's OCCT measurements.
  Also: §3 layout amended — core/src/cli/ added for the M6.2 executable and
  job driver. MC winding decision: exported meshes are currently wound inward;
  flip to outward as a small follow-up task before M6.3.
- 2026-07-10: M6.3 benchmarks regenerated by reviewer's independent reference (projected formulation). Three measured spec rulings locked in the fixture: unclamped OC ratio with 0/0-hold; rmin 2.5; β capped at 32 with damped move schedule (ROADMAP amended to match). Gate V2's test moves to the new formulation as part of M6.3.
- 2026-07-10: Optimizer roadmap clarified. "Discovers ribs/structure a user
  wouldn't design" is a DESIGN-DOMAIN capability (M7.dom, pure physics),
  available before and independent of any ML. ML (M7.ml) addresses SPEED/
  FRICTION, not capability: ML proposes a warm start, a real FEA pass certifies
  every user-facing result — certification is non-removable, ML is never the
