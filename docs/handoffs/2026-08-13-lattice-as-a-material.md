...
| **R2** Mode 2 off until Mode 1 measured; Mode 1 off until bounds met | **held, and re-checked after the ladder wiring** — `frozen_lattice` defaults false (`pipeline.hpp:552`), `frozen_lattice_beta` defaults empty (`:574`), nothing in `core/src/cli` or the app sets either, and the Mode 2 coupling is constructed ONLY when a beta field is present AND a surviving region is `Optimised`. Wiring Mode 2 in did not arm it |
...
| **R4** NET, and margin as a curve | ★ **HELD IN FULL.** NET at both rungs — −170.840 g (−31.4%) at 0.68 and −169.297 g (−47.0%) at 0.26, across the freed-mass frontier with a re-optimised control (§0.4, §0.4b). ★ And the margin CURVE is now produced (§0.4d): 127 iterations certified one by one, **settling at iteration 12** under B6's pre-registered rule, with the rebuilt posture checked against the run's own margin to 0.298% before the curve is presented at all |
