...
`s0_depth/s0_table.txt`, regenerated by `s0_table.py` (every figure READ from a
receipt; where a receipt carries none, the cell says so).

**Arm A reproduces his run.** At rung 0.68:

| | latticed voxels | region voxels | solid fallback | cell | mass |
|---|---|---|---|---|---|
| **A — his run** | **370** | 12,817 | 12,447 | 4.603 mm | 536.9 g |

and `include_void_by_optimizer` = **120,726**, against the **120,821** he
reported. That is his part, his faces, his settings, and his failure: the
declared regions are almost entirely void the optimizer removed, and of the
12,817 voxels that did have material, 12,447 fell back to SOLID with core's
reason `member_too_thin_for_cell` — a smaller cell helps.

**STATUS AT WRITING: arms B and C were still generating their rung-0.68
lattices.** Each arm walks the full production 4-rung ladder at resolution 128 —
...
ladder + margin apply") — so there is no short version of this measurement. Rerun
`s0_table.py` for the completed table; the barrier table above is complete and is
what §0(a) and §0(b) ask for.

**What is already settled without waiting**: the barrier holds 58.2% more
material at the matched depth; arm A reproduces his numbers to within 0.1% on the
include-void count; and core's own pre-flight says the CELL, not the depth, is
what left five of his eight regions solid.
