...
### §0 result table (the three arms, end to end)

See `s0_depth/s0_table.txt`. **STATUS AT WRITING: the three ladders were still
running.** Each walks the full production 4-rung ladder at resolution 128 —
core refuses a shortened `ladder` alongside `loads` ("in loadcase mode the
anchors are the fixtures, the groups are the design load, and the production
ladder + margin apply"), so there is no short version of this measurement, and
rung 0.68 alone took 28 minutes with three arms sharing the machine. The
loadcase-stage table above is complete and is the measurement §0(a) and §0(b) ask
for; the latticed-voxel / share / mass comparison against his 13,034 / 12% / 507 g
lands when the arms finish and belongs in `s0_table.txt`, not in a claim here.

**What is already settled without waiting**: the barrier holds 58.2% more
material at the matched depth, and core's own pre-flight says the cell — not the
depth — is what was leaving five of his eight regions solid.
...
* **(c) Auto never refuses.** The two combinations core rejects outright — `fit`
  with no include region, and `fit` alongside sub-floor retention — are
  unreachable from Auto; it drops the mutually-exclusive one and says so. A
  region that cannot be certified at any cell still gets the buildable cell and
  is reported out of regime.
...
| bar | status |
|---|---|
| **R1** demonstrably usable on his job | **PARTIAL.** The path is reachable end to end and the barrier is measured (+58.2% held). The latticed-voxel / share / mass comparison against 13,034 / 12% / 507 g was still on the ladder at writing — see §0 and `s0_table.txt`. |
...
| **R4** preview latency per interaction | **MET.** Table above; artifact `r4_preview_latency.txt`. |
...
While measuring it, your own run told us something bigger. Even with the depths
matched, five of your eight regions were still coming back solid — not because
the protection was too shallow, but because the CELL was too big. A 7 mm wall at a
4.6 mm cell is one and a half cells thick, and the certifier needs five. Your run
had been printing that explanation all along, along with the fix. So "Auto" now
does what it says: it asks the core to pick a cell per region from that region's
own thickness, which for your walls is 1.4 mm and certifies. Press Auto on
everything and it works, and where it genuinely cannot certify it builds the
buildable thing and tells you which region is out of regime instead of stopping.
