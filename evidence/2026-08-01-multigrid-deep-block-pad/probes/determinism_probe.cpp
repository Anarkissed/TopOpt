// P7 determinism (task: multigrid-deep-block-pad) — a pad-rescued all-even
// DEEP-BLOCKED solve (30^3: every axis even, but 15^3 after one halving is odd
// AND over the DOF cap) is bit-identical run-to-run and within-process repeat,
// at 1, 2, 4, 8 matvec threads. Same probe as 262's O9, deep-blocked grid.
...
int main() {
  VoxelGrid g;
  g.nx = 30; g.ny = 30; g.nz = 30;  // all even, deep-blocked -> pad engages
  g.spacing = 1.0;
  g.origin = Vec3{0, 0, 0};
