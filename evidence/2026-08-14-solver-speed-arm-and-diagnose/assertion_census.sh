#!/usr/bin/env bash
# assertion_census.sh — R6, done as a MESSAGE census rather than a name grep.
#
...
#      refusal.
#
# ★ THIS TASK'S DIFF TOUCHES core/src AND core/include, so section 3 is the
# load-bearing one: `multigrid.cpp` gained a measurement-only re-arm branch and
# `fea.hpp` gained four declarations, and neither may have cost a refusal or a
# static_assert. Section 6 additionally censuses the STATIC_ASSERT MESSAGES in
# core/src, which is where multigrid.cpp keeps the tripwires that bind
# `MgTuning`'s defaults to the shipped constants — a compile-time assertion is
# still an assertion and the four sections above cannot see it.
#
