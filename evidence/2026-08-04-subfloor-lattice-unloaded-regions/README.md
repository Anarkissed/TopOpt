...
| `s9_determinism_and_ladder_coupling.txt` | **S9**, and the control that attributes §7's finding: the same job run twice is byte-identical, so the divergence of later rungs when retention is armed (40 / 380 / 416 classification flips) is CAUSED, not noise. |
