#!/usr/bin/env bash
# assertion_census.sh — R8, done as a MESSAGE census rather than a name grep.
#
# ★ CARRIED ACROSS FROM PR 324's OWN CENSUS, with one addition: this task's diff
# is mostly in core/src and core/include, and THREE HARNESS HEADERS WERE REDUCED
# TO SHIMS. Section 5 counts the harness as one bag, which is exactly right for a
# move OUT of the harness into core — a refusal dropped on the way would show as
# REMOVED there and as present nowhere. Section 3 (production refusals) is where
# the moved code's refusals must now appear.
#
...
#   5. ★ THE HARNESS'S OWN REFUSALS. This task's whole diff is in
#      `core/tests/harness/`, and two of its files are MOVES of code out of
#      existing probes. A move that dropped a `FATAL:` or a bounds check on the
#      way would be invisible to 1-4, which only look at core/src and
#      core/tests/unit. So the harness's `FATAL:` and `throw` messages are
#      censused too, and the two moved headers are counted as part of the files
...
echo "=== R8 assertion census: $BASE -> working tree ==="
echo
...
# ★ THE AFTER BAG INCLUDES CORE'S NEW HEADERS. Three harness headers were MOVED
# into core/include/topopt this task, so a message that used to live in the
# harness and now lives in core has not disappeared — it moved. Counting only
# core/tests/harness would report every one of them as REMOVED and drown the
# signal.
cat core/tests/harness/*.cpp core/tests/harness/*.hpp core/tests/harness/*.inc \
    core/include/topopt/plsm_basis.hpp core/include/topopt/plsm_kernel.hpp \
    core/include/topopt/plsm_mma.hpp core/include/topopt/plsm.hpp \
    core/src/simp/plsm.cpp \
