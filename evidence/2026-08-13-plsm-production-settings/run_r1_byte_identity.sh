...
# what a byte-identity check catches and what an argument does not: while wiring
# item 1 it turned out `JobDescription` duplicated `PlsmOptions`' defaults as
# literals, so the production edit alone would have been a no-op for every job
# that carries a plsm block. The reverse failure — a PLSM default reaching the
# SIMP path — is what this script rules out.
#
# ★ WHAT IS AND IS NOT EXPECTED TO MOVE. The job document below carries NO
# "plsm" block, so `PlsmMode::Off` holds and every number must be identical. A
# job that DOES carry one is expected to move: that is items 1-3, and the arms
# measure it. The bar is not "nothing changed"; it is "nothing changed where
# nothing should".
#
...
mkdir -p "$OUT"
STASH_MSG="plsm-production-settings-r1-$$"
