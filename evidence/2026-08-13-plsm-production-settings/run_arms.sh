...
#
#   B_heaviside   the PREVIOUS production posture: H_eta at the cell centre,
#                 eta = 2 voxels, cap 60, no margin probe.
#   C_eta1        + eta = 1.                            ITEM 1, ALONE.
#   D_fraction    + the exact volume fraction, k = 4.    ITEM 2, ALONE, AT A
#                                                        MATCHED 60-ITERATION
#                                                        BUDGET.
...
#
# ★ 3 THREADS, STRICTLY SERIAL. He needs his machine, and two arms sharing a
# host cannot have comparable wall clocks (this Mac's own A/B offset is larger
# than 10%).
set -e
...
arms = {
  ...
  "B_heaviside": {**common, "ersatz": "heaviside", "eta_voxels": 2.0,
                  "max_iterations": 60, "margin_probe_every": 0},
  # item 1, alone
  "C_eta1":      {**common, "ersatz": "heaviside", "eta_voxels": 1.0,
                  "max_iterations": 60, "margin_probe_every": 0},
  # item 2, alone, at the SAME budget
  "D_fraction":  {**common, "ersatz": "fraction", "eta_voxels": 1.0,
                  "frac_samples": 4, "max_iterations": 60,
                  "margin_probe_every": 0},
  # item 3, and the posture that ships
  "A_ship":      {**common, "ersatz": "fraction", "eta_voxels": 1.0,
                  "frac_samples": 4, "max_iterations": 120,
...
for arm in $ARMS; do
  ...
  ./build/topopt-cli run "$SCRATCH/jobs/plsm_$arm.json" \
      --out "$SCRATCH/$arm" --materials "$MATS" --threads 3 \
