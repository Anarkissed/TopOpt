#!/usr/bin/env bash
# R6 -- NEVER WEAKEN OR DELETE AN ASSERTION, as a MESSAGE census rather than a
...
#
# THIS TASK'S S1 CHANGES NO PRODUCTION FILE AND NO TEST. It adds one
# EXCLUDE_FROM_ALL harness and the CMake block declaring it. So every "removed"
# list below is expected to be EMPTY, and an empty list is the bar being met --
# not the script failing to look.
#
#   ./evidence/2026-08-08-how-wrong-is-the-boundary-cell/assertion_census.sh
set -uo pipefail
...
echo "=== VERDICT: $([ "$FAIL" -eq 0 ] && echo 'R6 PASS -- nothing was weakened or deleted' || echo 'R6 ATTENTION -- see the *** lines above') ==="
exit 0
