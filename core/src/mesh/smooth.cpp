...
// centroid of its 1-ring neighbours' CURRENT positions by s; frozen vertices are
// copied verbatim (bit-identical). Reads `pos`, writes `next` (same size).
void laplacian_pass(const std::vector<Vec3>& pos,
                    const std::vector<std::vector<int>>& adj,
                    const std::vector<char>& frozen, double s,
                    std::vector<Vec3>& next) {
  for (std::size_t v = 0; v < pos.size(); ++v) {
    if (frozen[v] || adj[v].empty()) {
      next[v] = pos[v];  // frozen (or isolated): verbatim
      continue;
    }
    ...
    const Vec3 lap{sum.x * inv - pos[v].x, sum.y * inv - pos[v].y,
                   sum.z * inv - pos[v].z};
    next[v] = Vec3{pos[v].x + s * lap.x, pos[v].y + s * lap.y,
                   pos[v].z + s * lap.z};
  }
}
...
SmoothResult constrained_taubin_smooth(const TriangleMesh& mesh,
  ...
  st.volume_drift_bound = taubin_volume_drift_bound(params);
  ...
  for (int p = 0; p < params.pairs; ++p) {
    ...
    laplacian_pass(pos, adj, frozen, params.lambda, tmp);
    laplacian_pass(tmp, adj, frozen, mu, next);
